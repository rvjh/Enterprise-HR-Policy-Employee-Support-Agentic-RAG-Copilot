...
def llm() -> ChatGroq:
    """Instantiate and cache the ChatGroq model."""
    global _llm
    if _llm is None:
...
def web_search_tool():
    ...
    if _web_search is None:
        if not settings.tavily_api_key:
            raise RuntimeError("TAVILY_API_KEY is missing from configuration")
...
Return "good" if the evidence is sufficient and directly relevant; otherwise return "weak".
"""
    )
    return {
...
def direct_answer(state: AgentState):
    response = llm().invoke(f"Respond briefly and naturally to: {state['question']}")
    return {
        "answer": response.content,
        "source_used": "direct",
...
def insufficient(state: AgentState):
    return {
        "answer": (
            "I couldn't find enough reliable evidence in the company HR knowledge base "
            "or external search to answer confidently. Please contact the HR team or provide more details."
        ),
